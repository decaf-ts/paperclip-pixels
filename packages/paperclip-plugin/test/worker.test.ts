...
describe("worker trust boundary (criterion 8)", () => {
  it("routes the relay's outbound push through the SDK-gated ctx.http.fetch surface (not a bare global fetch)", async () => {
    ...
    for (const [url, init] of httpFetch.mock.calls as Array<[string, RequestInit?]>) {
      expect(url).toBe("https://pa.example/api/hooks/paperclip-bridge");
...
describe("worker relay wiring (SAA-229 coverage gap)", () => {
  ...
  it("setupCompany configures the relay from ctx.config and spawns a session per agent via ingestSnapshot", async () => {
    ...
    expect(relayFetchCalls.map((c) => JSON.parse(c.body).sessionId).sort()).toEqual([
      "paperclip-bridge:company-acme:agent-ceo",
      "paperclip-bridge:company-acme:agent-dev",
    ]);
  });

  it("boots with the relay disabled and a warning-level log when no relay config is present", async () => {
    const { harness } = seedStandardWorld();
    await setupWorker(harness);
  ...
  it("the event handler forwards the canonical event to the relay after store.applyPaperclipEvent", async () => {
    ...
    // The unseen agent spawns exactly one sessionStart push on the relay.
    expect(relayFetchCalls.length).toBe(baseline + 1);
    const last = relayFetchCalls[relayFetchCalls.length - 1];
    const body = JSON.parse(last.body);
    expect(body.sessionId).toBe("paperclip-bridge:company-acme:agent-relay-late");
    expect(body.event).toEqual({ kind: "sessionStart", source: "paperclip-bridge" });
