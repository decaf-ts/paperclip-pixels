...
import type { AgentInput } from "@paperclip-pixel/core";
...
describe("BridgeTransport (§31.4, §30)", () => {
  ...
  it("reconnect() without a snapshot resumes flow but leaves the sidecar stale", () => {
    ...
    const newStarts = sink.events
      .slice(before)
      .filter((e) => e.sessionId === sessionId(AGENT_NEW));
...
describe("HttpPushSink (§31.4, §7.3)", () => {
  ...
  it("POSTs the envelope to the bridge endpoint with a bearer token", async () => {
    ...
    expect(url).toBe("https://bridge.example/api/hooks/paperclip-bridge");
  ...
  it("honors a custom providerId in the URL and envelope", async () => {
    const fetchMock = fetchLike();
    fetchMock.mockResolvedValue({ ok: true, status: 200, statusText: "OK" });
    const sink = new HttpPushSink({
      baseUrl: "https://bridge.example",
    ...
    expect(JSON.parse(init.body).providerId).toBe("custom-provider");
  });
