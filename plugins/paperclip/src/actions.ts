...
// R3-WS5b: set one agent's v2 appearance assignment (whole-sheet or
// composition mode). The `appearance` payload is validated client-agnostically
// via the common `validateAppearanceAssignmentV2` against the composition part
// catalog (a whole-sheet branch needs no catalog; a composition branch is
// validated fail-closed). `.strict()` rejects a forged `appearance` that is not
// a plain object-shaped payload.
const SetAgentCompositionSchema = z.object({
  companyId: z.string().min(1),
  agentId: z.string().min(1),
...
// R3-WS4a: Paperclip hosts the declarative office-layout configuration. The
// layout payload is validated client-agnostically via the common Zod schema
// (validateOfficeLayout) after the host-scoped companyId is resolved. `.strict()`
// rejects a forged `layout` that is not a plain object-shaped payload.
const SetOfficeLayoutSchema = z.object({
  companyId: z.string().min(1),
  layout: z.unknown(),
  renderEnvironment: z.unknown().optional(),
}).strict();
...
export interface ActionDeps {
  ...
  /**
   * Persist and apply one agent's v2 appearance assignment (R3-WS5b
   * composition). Implemented by the worker: persists the v2 assignment
   * (whole-sheet or composition mode) to plugin `ctx.state` agent scope and
   * updates the runtime v2 map. The overall-sheet relay push is left
   * untouched — composed-appearance rendering/validation is the Pixel Agents
   * plugin's job, not this one (board Revision 3 boundary).
   */
  applyAgentAppearanceV2?: (
    companyId: string,
    input: { agentId: string; agentName: string; appearance: AppearanceAssignment },
  ) => Promise<{ ok: boolean; error?: string; appearance?: AppearanceAssignment; applied?: boolean }>;
}
...
export async function handleSetAgentComposition(
  deps: ActionDeps,
  params: Record<string, unknown>,
  context: PluginPerformActionContext,
): Promise<unknown> {
  const parsed = SetAgentCompositionSchema.safeParse(params);
  if (!parsed.success) return { ok: false, error: "INVALID_PARAMS", details: parsed.error.issues };
  const scope = resolveCompanyScope(parsed.data.companyId, context);
  if ("error" in scope) return { ok: false, error: scope.error };
  // Validate the v2 appearance against the composition part catalog. A
  // whole-sheet branch needs no catalog; a composition branch is validated
  // fail-closed (unknown part, duplicate single-kind, hue/palette overflow).
  let catalog;
  try {
    catalog = loadCompositionPartCatalog().catalog;
  } catch {
    catalog = null;
  }
  if (catalog) {
    const violations = validateAppearanceAssignmentV2(parsed.data.appearance, catalog);
    if (violations.length > 0) {
      return { ok: false, error: "INVALID_APPEARANCE", violations };
    }
  }
  const agent = await deps.ctx.agents.get(parsed.data.agentId, scope.companyId);
  if (!agent) return { ok: false, error: "AGENT_NOT_FOUND" };
  if (!deps.applyAgentAppearanceV2) return { ok: false, error: "APPEARANCE_APPLIER_UNAVAILABLE" };
  return deps.applyAgentAppearanceV2(scope.companyId, {
    agentId: agent.id,
    agentName: agent.name,
    appearance: parsed.data.appearance as AppearanceAssignment,
  });
}

export async function handleSetOfficeLayout(
  deps: ActionDeps,
  params: Record<string, unknown>,
  context: PluginPerformActionContext,
): Promise<unknown> {
  const parsed = SetOfficeLayoutSchema.safeParse(params);
  if (!parsed.success) return { ok: false, error: "INVALID_PARAMS", details: parsed.error.issues };
  const scope = resolveCompanyScope(parsed.data.companyId, context);
  if ("error" in scope) return { ok: false, error: scope.error };
  const validated = validateOfficeLayout(parsed.data.layout);
  if (!validated.ok) {
    return { ok: false, error: "INVALID_LAYOUT", errors: validated.errors };
  }
  await persistOfficeLayout(deps.ctx, scope.companyId, validated.layout);
...
export function registerActions(deps: ActionDeps): void {
  ...
  deps.ctx.actions.register(ACTION_KEYS.setAgentComposition, (params, context) =>
    handleSetAgentComposition(deps, params, context),
  );
  deps.ctx.actions.register(ACTION_KEYS.setOfficeLayout, (params, context) =>
    handleSetOfficeLayout(deps, params, context),
  );
}
