...
// --- R3-WS5b: per-agent v2 appearance assignment (composition) ---------------

/**
 * Persist one agent's v2 appearance assignment (whole-sheet or composition
 * mode) in the plugin SDK's `agent` state scope. This is the single source of
 * truth for the composer; the whole-sheet `agent-character` record remains
 * the frozen WS3 contract the relay reads.
 */
export async function persistAgentAppearanceV2(
  ctx: PluginContext,
  agentId: string,
  assignment: AgentAppearanceAssignmentV2,
): Promise<void> {
  await ctx.state.set(agentScope(agentId, STATE_KEYS.agentComposition), assignment);
}

/**
 * Load one agent's v2 appearance assignment. Returns `null` when absent or
 * malformed (fail-closed to the whole-sheet default rather than serving an
 * unvalidated shape to the composer).
 */
export async function loadAgentAppearanceV2(
  ctx: PluginContext,
  agentId: string,
): Promise<AgentAppearanceAssignmentV2 | null> {
  const data = await ctx.state.get(agentScope(agentId, STATE_KEYS.agentComposition));
  if (data == null) return null;
  const parsed = AgentAppearanceAssignmentV2Schema.safeParse(data);
  return parsed.success ? data as AgentAppearanceAssignmentV2 : null;
}

/**
 * Bulk-load the per-agent v2 assignment map for a known set of agent ids.
 * Malformed entries are skipped (fail-closed to defaults).
 */
export async function loadAgentAppearanceV2Map(
  ctx: PluginContext,
  agentIds: string[],
): Promise<Record<string, AgentAppearanceAssignmentV2>> {
  const map: Record<string, AgentAppearanceAssignmentV2> = {};
  for (const agentId of agentIds) {
    const assignment = await loadAgentAppearanceV2(ctx, agentId);
    if (assignment) map[agentId] = assignment;
  }
  return map;
}

// --- R3-WS4a: bounded relay-communications log + office-layout config -------

/** Persist a company's bounded relay-communications log for restart recovery. */
export async function persistRelayComms(
  ctx: PluginContext,
  companyId: string,
  compact: CompactRelayComms,
): Promise<void> {
  await ctx.state.set(companyScope(companyId, STATE_KEYS.relayComms), compact);
}

/**
 * Load a company's relay-communications log. Returns `null` when absent or
 * malformed (fail-closed to a fresh bounded log rather than serving an
 * unvalidated shape).
 */
export async function loadRelayComms(
  ctx: PluginContext,
  companyId: string,
): Promise<CompactRelayComms | null> {
  const data = await ctx.state.get(companyScope(companyId, STATE_KEYS.relayComms));
  if (data == null) return null;
  if (typeof data !== "object" || Array.isArray(data)) return null;
  const compact = data as CompactRelayComms;
  if (typeof compact.limit !== "number" || !Array.isArray(compact.entries)) return null;
  return compact;
}

/** Persist the Paperclip-hosted office-layout configuration for a company. */
export async function persistOfficeLayout(
  ctx: PluginContext,
  companyId: string,
  layout: OfficeLayout,
): Promise<void> {
  await ctx.state.set(companyScope(companyId, STATE_KEYS.officeLayout), layout);
}

/**
 * Load the Paperclip-hosted office-layout configuration. Returns `null` when
 * absent or malformed (fail-closed — the hosting surface treats an unreadable
 * layout as "no layout configured" rather than serving an unvalidated shape).
 */
export async function loadOfficeLayout(
  ctx: PluginContext,
  companyId: string,
): Promise<OfficeLayout | null> {
  const data = await ctx.state.get(companyScope(companyId, STATE_KEYS.officeLayout));
  if (data == null) return null;
  return isOfficeLayout(data) ? data : null;
}
