...
export class AgentWindowStore {
  ...
  /**
   * Time-bucketed series (R3-WS4a): the retained 5-minute buckets whose span
   * overlaps [now - windowMs, now], in oldest-first order. This is the
   * chartable series surface — every bucket carries the per-bucket
   * communication and operational counters the UI plots natively. Unlike
   * {@link getWindowedMetrics} (a single aggregate), this returns the raw
   * buckets so a consumer can chart the shape across time.
   */
  getBucketsInWindow(agentId: string, now: number, windowMs: number): Bucket[] {
    return this.recentBuckets(agentId, now, windowMs);
  }
