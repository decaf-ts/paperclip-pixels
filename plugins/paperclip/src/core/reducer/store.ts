...
import { BUCKET_INTERVAL_MS, TIME_WINDOWS, WINDOW_DURATION_MS } from "../domain/metrics.js";
import type { Bucket } from "../temporal/windows.js";
import type {
  ApprovalInput,
...
/**
 * One time-bucketed metrics point (R3-WS4a). This is the chartable shape the
 * UI plots: a stable, documented subset of the aggregate {@link WindowedMetrics}
 * broken out per 5-minute bucket, with the bucket start as an ISO timestamp.
 */
...
/** One agent's time-bucketed series. */
...
/**
 * The stable, documented metrics-series query shape served to the UI (R3-WS4a).
 * `agents` carries one series per agent with data in the window; `company` is
 * the same series summed across every agent (oldest-first). `buckets` is
 * ordered oldest-first so the UI plots left-to-right without re-sorting.
 */
...
export class BridgeStore {
  ...
  /**
   * Time-bucketed metrics series (R3-WS4a). When `agentId` is given, returns
   * that one agent's series; otherwise every agent with data in the window. A
   * company-wide `company` series is always included (summed across agents,
   * oldest-first). The result carries a stable, documented shape the UI charts
   * natively — empty series are represented by an empty `buckets` array rather
   * than a missing key.
   */
  getMetricsSeries(
    agentId: string | undefined,
    window: TimeWindow,
    now = this.lastObservedAtMs || Date.now(),
  ): MetricsSeriesResult {
    const windowMs = WINDOW_DURATION_MS[window];
    const included = agentId
      ? this.state.agents.has(agentId)
        ? [agentId]
        : []
      : [...this.state.agents.keys()];

    const series: AgentMetricsSeries[] = [];
    const aggregate = new Map<number, MetricsBucket>();
    for (const id of included) {
      const buckets = this.windows
        .getBucketsInWindow(id, now, windowMs)
        .map(collapseBucket);
      series.push({ agentId: id, buckets });
      for (const b of buckets) {
        const prev = aggregate.get(toEpoch(b.bucketStart));
        aggregate.set(toEpoch(b.bucketStart), prev ? addMetricsBucket(prev, b) : b);
      }
    }
    const company = [...aggregate.entries()]
      .sort((a, b) => a[0] - b[0])
      .map(([, b]) => b);
...
function collapseBucket(b: Bucket): MetricsBucket {
  return {
    bucketStart: new Date(b.bucketStart).toISOString(),
...
function toEpoch(bucketStart: string): number {
  const ms = new Date(bucketStart).getTime();
  return Number.isFinite(ms) ? ms : 0;
}

function addMetricsBucket(a: MetricsBucket, b: MetricsBucket): MetricsBucket {
  return {
    bucketStart: a.bucketStart,
    runStarts: a.runStarts + b.runStarts,
    runFinishes: a.runFinishes + b.runFinishes,
    runFailures: a.runFailures + b.runFailures,
    runCancellations: a.runCancellations + b.runCancellations,
    issueTransitions: a.issueTransitions + b.issueTransitions,
    projectSwitches: a.projectSwitches + b.projectSwitches,
    commentEvents: a.commentEvents + b.commentEvents,
    questionEvents: a.questionEvents + b.questionEvents,
    blockedEvents: a.blockedEvents + b.blockedEvents,
    approvalWaitEvents: a.approvalWaitEvents + b.approvalWaitEvents,
