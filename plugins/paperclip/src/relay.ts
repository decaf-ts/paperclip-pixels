...
export interface RelayCompanyConfig {
  ...
  /**
   * Bounded rolling relay-communications log capacity (R3-WS4a): number of
   * relay/feed communications retained per company, oldest evicted. Defaults to
   * {@link DEFAULT_RELAY_COMMS_LIMIT} (100); strictly bounded, never unbounded.
   */
  relayCommsLimit: number;
}
...
export function parseRelayConfig(
  ...
  // R3-WS4a: bounded rolling relay-communications log capacity, strictly
  // bounded and clamped to a sane positive integer (the store itself fails
  // closed on nonsense values, but clamp here so the config the rest of the
  // worker sees is always coherent). Defaults to 100.
  const relayCommsLimit = parseCommsLimit(raw.relayCommsLimit);
  return {
    enabled,
    pixelAgentsUrl: url,
    pixelAgentsUiUrl,
...
/** Normalize an operator-supplied relay-communications limit to a valid
 *  positive integer; falls back to {@link DEFAULT_RELAY_COMMS_LIMIT}. */
export function parseCommsLimit(raw: unknown): number {
  const n = typeof raw === "number" ? raw : Number(raw);
  if (!Number.isFinite(n) || n < 1) return DEFAULT_RELAY_COMMS_LIMIT;
  return Math.floor(n);
}
...
export class BridgeRelay {
  ...
  private commsRecorder?: (companyId: string, operations: PluginFeedOperation[]) => void;
  ...
  /**
   * Register a callback invoked with each feed-operation batch the relay
   * enqueues for a company (R3-WS4a). The worker uses it to append every
   * relay/feed communication to the bounded comms store. Called before the
   * sink push so the log reflects what was actually sent.
   */
  setCommsRecorder(recorder: (companyId: string, operations: PluginFeedOperation[]) => void): void {
    this.commsRecorder = recorder;
  }

  /** Record a relay/feed communication (no-op when no recorder is registered). */
  private recordComms(companyId: string, operations: PluginFeedOperation[]): void {
    this.commsRecorder?.(companyId, operations);
  }
  ...
  async configure(companyId: string, raw: Record<string, unknown>): Promise<void> {
    ...
    if (
      ...
      && existing.config.relayCommsLimit === config.relayCommsLimit
    ) {
  ...
  ingestSnapshot(companyId: string, snapshot: AuthoritativeSnapshotInput): void {
    ...
    if (relay) {
      const operations = relay.mapper.mapSnapshot(snapshot);
      this.recordComms(companyId, operations);
      void relay.sink.push(companyId, operations);
    }
  }
  ...
  ingestEvent(companyId: string, event: BridgeInputEvent): void {
    const relay = this.companies.get(companyId);
    if (relay) {
      const operations = relay.mapper.mapEvent(event);
      this.recordComms(companyId, operations);
      void relay.sink.push(companyId, operations);
    }
  }
  ...
  async syncAppearances(
    ...
    if (assignments.length > 0) {
      const operations = relay.mapper.setAppearances(assignments.map(toFeedAppearance));
      this.recordComms(companyId, operations);
      void relay.sink.push(companyId, operations);
    }
    return true;
