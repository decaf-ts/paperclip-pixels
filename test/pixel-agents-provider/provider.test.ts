...
describe("PaperclipBridgeProvider contract (§31.4, §27.3, §39.1)", () => {
  ...
  it("normalizeHookEvent returns the primary event (toolStart) on first sight, not the leading sessionStart", () => {
    const p = provider();
    const raw = {
      eventId: "ev-2",
      timestamp: new Date(0).toISOString(),
    ...
    // agent.run.started now has a genuine primary AgentEvent (§21.4): the
    // contract method returns the LAST mapped event, i.e. toolStart, not the
    // leading sessionStart (the transport's multi-emit path still delivers both).
    expect(normalized!.event.kind).toBe("toolStart");
  });
  ...
  it("maps a typed run.started into sessionStart + toolStart('PaperclipWork'), never a fabricated real tool", () => {
    const p = provider();
    ...
    expect(res.sidecar?.kind).toBe("run-activity");
    const toolStart = res.agentEvents[1].event as { kind: "toolStart"; toolName: string };
    expect(toolStart.toolName).toBe("PaperclipWork");
  });
});
