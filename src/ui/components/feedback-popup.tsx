...
import type { AgentFeedback } from "../../core/index.js";
...
// Worker reply surface (bridge contract, src/actions.ts handleAgentReplyToFeedback):
// - success: `{ ok: true, feedbackId, issueId, runId }`
// - reroute: `{ ok: false, error: "ROUTE_TO_COMPANY", reason, suggestedText }`
// - rejection (e.g. INVALID_PARAMS): `{ ok: false, error: "…", details }`
// `kind` is accepted for legacy/compat mock callers.
...
export function FeedbackPopup({
  ...
  async function handleReply() {
    ...
    try {
      ...
      const routedOutcome =
        (result && result.kind === "route-to-company") ||
        (result && result.error === "ROUTE_TO_COMPANY");

      if (result && routedOutcome) {
        setError(
      ...
      // Any other non-success outcome (eg. `{ ok: false, error: "INVALID_PARAMS" }`)
      // means the reply was NOT sent — surface it as an error instead of
      // pretending the reply succeeded (SAA-339 Defect 2).
      if (!result || result.ok === false) {
        setError(
          result && result.error
            ? `The reply was not sent: ${result.error}`
